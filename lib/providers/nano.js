...
const NEEDS_DOWNLOAD_MSG =
  "Ballpark's free on-device model needs a one-time download before it can calibrate. Enable it from the Ballpark popup.";
const UNAVAILABLE_MSG =
  "This device can't run Ballpark's free on-device model. Add an API key in the popup to calibrate.";
...
export async function calibrate(payload, config) {
  ...
  if (state === 'unavailable') return { status: 'unavailable', provider: id, message: UNAVAILABLE_MSG };
  if (state !== 'available') {
