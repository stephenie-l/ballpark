...
(function () {
  ...
  // Version numbers: v2.5, 2.5.1. Anchored at both ends so it only matches a
  // token that is ENTIRELY a version — otherwise it would swallow the leading
  // "N.N" of any decimal (4.5%, 2.3M, 2.3 billion) and wrongly exclude it.
  ...
  function shouldSkipElement(el) {
    ...
    // Skip if inside a link, or inside our own calibration card (#bp-card) —
    // the re-scan observer fires when the card mounts, and we must not underline
    // the numbers in the card's own verdict/comparisons.
    if (el.closest && el.closest('a, code, pre, #bp-card')) return true;
    return false;
  }
